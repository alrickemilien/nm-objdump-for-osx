...
class TestOtool < Test::Unit::TestCase
	...
	# This test is not triggered because the real otool loops on /dev/random
	#def test_on_dev_zero
	#	diff("#{OTOOL} /dev/zero", "#{FT_OTOOL} /dev/zero")
	#end
	...
	def test_corrupted_archives
		corrupted_archives_samples = Dir["#{__dir__}/corrupted_samples/archives"].join(" ")
		diff("#{OTOOL} -t #{corrupted_archives_samples}", "#{FT_OTOOL} #{corrupted_archives_samples}")
	end
	...
	#	diff("#{OTOOL} /dev/random", "#{FT_OTOOL} /dev/random")
	#end
