...
void	*map_loading_file(const char *filename, uint64_t *file_size)
{
  ...
  if ((fstat(fd, &stats)) == -1)
    return (mach_o_error(0, OTOOL_DEFAULT_MACHO_ERROR, filename));
  ...
  *file_size = (off_t)stats.st_size;
  ...
  if (S_ISDIR(stats.st_mode))
    return (mach_o_error(0, "%s: Is a directory.\n", filename));

  if ((stats.st_mode & S_IFMT) != S_IFREG && (stats.st_mode & S_IFMT) != S_IFLNK)
    return (mach_o_error(-1, OTOOL_DEFAULT_MACHO_ERROR, filename));
